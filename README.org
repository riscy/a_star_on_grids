...
* Preliminaries
  ...
  f_i = g_i + h_i
  #+end_src
  which is an estimate of the cost of a path going through ~i~ and arriving at
  the goal.  Here ~g_i~ is the cost of the cheapest path to state ~i~ that A*
  has generated so far, and ~h_i~ is a cheaply computed /heuristic estimate/ for
  the cost to get from ~i~ to the goal.
...
*** Use a binary heap
    ...
    for small grids -- on the order of a couple thousand states in a C++
    implementation.  On grids with very few obstacles, maintaining the heap
    might be more expensive than linear scans of the open list.  (See [[https://github.com/riscy/a_star_on_grids/blob/master/src/node_heap.h][example
    code]].)
...
*** Avoid recomputing heuristics
    ...
    #+end_src
    (See [[https://github.com/riscy/a_star_on_grids/blob/master/src/algorithms.cpp#L147][example code]].)
